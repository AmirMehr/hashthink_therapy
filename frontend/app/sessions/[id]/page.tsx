    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        <div className="grid grid-cols-2 gap-4">
          <div><strong>ID:</strong> {session.sessionId}</div>
          <div><strong>Therapist:</strong> {session.therapistId}</div>
          <div><strong>Client:</strong> {session.clientId}</div>
          <div><strong>Started:</strong> {formatDate(session.startTime)}</div>
          <div><strong>Entries:</strong> {session.entries?.length || 0}</div>
        </div>
      </div>

      <TranscribeForm sessionId={session.sessionId} />
      <ActionButtons sessionId={session.sessionId} hasSummary={!!session.summary} />
          ...
          <div className="space-y-2">
            {session.entries.map((entry, i) => (
              <div key={i} className="p-3 bg-gray-50 rounded">
                <strong className="text-blue-600">{entry.speaker}:</strong> {entry.content}
              </div>
            ))}
          </div>
        </div>
